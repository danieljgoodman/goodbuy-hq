...
describe('ValuationResults Component', () => {
  ...
  describe('Tab Navigation', () => {
    it('renders all tabs when business data is not provided', () => {
      ...
      expect(
        screen.getByRole('button', { name: /valuation methods/i })
      ).toBeInTheDocument()
      expect(
        screen.getByRole('button', { name: /key metrics/i })
      ).toBeInTheDocument()
      expect(
    ...
    it('renders AI Analysis tab when business data is provided', () => {
      render(
        <ValuationResults result={mockResult} businessData={mockBusinessData} />
      )
    ...
    it('switches between tabs', async () => {
      ...
      const methodsTab = screen.getByRole('button', {
        name: /valuation methods/i,
      })
    ...
    it('shows recommendations tab content', async () => {
      ...
      const recommendationsTab = screen.getByRole('button', {
  ...
  describe('Methods Tab', () => {
    it('displays all valuation methods', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Market Multiple')).toBeInTheDocument()
        expect(
          screen.getByText('DCF analysis based on projected cash flows')
        ).toBeInTheDocument()
        expect(
          screen.getByText('Comparison with similar businesses in the market')
        ).toBeInTheDocument()
      })
    })
  ...
  describe('Recommendations Tab', () => {
    it('displays recommendations and risk factors', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('High dependency on single customer segment')
        ).toBeInTheDocument()
        expect(
          screen.getByText('Limited geographical diversification')
        ).toBeInTheDocument()
      })
    })
    ...
    it('shows fallback message when no recommendations', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Your business shows strong performance/)
        ).toBeInTheDocument()
        expect(
          screen.getByText(/No significant risk factors identified/)
        ).toBeInTheDocument()
      })
    })
  })
  ...
  describe('AI Analysis Tab', () => {
    it('displays AI analysis when business data is provided', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByTestId('business-analysis-panel')
        ).toBeInTheDocument()
        expect(
          screen.getByText('AI Analysis for Test Business Inc.')
        ).toBeInTheDocument()
      })
    })
  })
  ...
  describe('Responsive Behavior', () => {
    it('renders correctly on different screen sizes', () => {
      ...
      const container = screen
        .getByText('Business Valuation Report')
        .closest('div')
