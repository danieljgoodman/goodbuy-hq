...
class FinancialHealthTester {
  ...
  async test1_NavigateToFinancialHealthPage() {
    ...
    try {
      ...
      await this.takeScreenshot(
        ...
        'Initial navigation to financial health page'
      )
      ...
      if (
        ...
        this.testResults.passed.push(
          '✅ Correctly redirected to authentication'
        )
        console.log('✅ Correctly redirected to authentication')
      } else {
        this.testResults.warnings.push(
          '⚠️ Not redirected to authentication - might be already logged in'
        )
        console.log(
          '⚠️ Not redirected to authentication - might be already logged in'
        )
  ...
  async test3_VerifyPageLoadsWithoutErrors() {
    console.log(
      '🧪 Test 3: Verify page loads without React serialization errors'
    )
    ...
    try {
      ...
      await this.takeScreenshot(
        '05-page-loaded',
        'Financial health page fully loaded'
      )
      ...
      for (const errorPattern of reactErrors) {
        if (pageContent.toLowerCase().includes(errorPattern.toLowerCase())) {
          ...
          this.testResults.failed.push(
            `❌ React serialization error found: ${errorPattern}`
          )
      ...
      if (!hasReactErrors) {
        this.testResults.passed.push(
          '✅ No React serialization errors detected'
        )
      ...
      if (hasHeading) {
        const headingText = await this.page.$eval(
          'h1, h2',
          el => el.textContent
        )
      ...
      this.testResults.failed.push(
        `❌ Page load verification failed: ${error.message}`
      )
  ...
  async test4_VerifyBusinessListAndTechCorp() {
    console.log(
      '🧪 Test 4: Verify business list loads and TechCorp Solutions appears'
    )
    ...
    try {
      ...
      for (const selector of businessSelectors) {
        try {
          ...
          if (elements.length > 0) {
            ...
            console.log(
              `📋 Found ${elements.length} business elements with selector: ${selector}`
            )

            // Check if TechCorp is in the list
            const pageText = await this.page.content()
            if (
              pageText.includes('TechCorp Solutions') ||
              pageText.includes('TechCorp')
            ) {
              businessFound = true
              this.testResults.passed.push(
                '✅ TechCorp Solutions found in business list'
              )
      ...
      if (!businessFound) {
        ...
        if (
          pageText.includes('TechCorp') ||
          pageText.includes(this.expectedBusinessId)
        ) {
          businessFound = true
      ...
      if (!businessListFound) {
        this.testResults.warnings.push(
          '⚠️ No business list elements found with standard selectors'
        )
        console.log(
          '⚠️ No business list elements found with standard selectors'
        )
      ...
      if (!businessFound) {
        this.testResults.failed.push(
          '❌ TechCorp Solutions not found in business list'
        )
      ...
      const hasLoadingIndicator =
        (await this.page.$('.loading, .spinner, [data-loading="true"]')) !==
        null
      ...
      this.testResults.failed.push(
        `❌ Business list verification failed: ${error.message}`
      )
  ...
  async test5_VerifyAnalyzeHealthButton() {
    console.log(
      '🧪 Test 5: Verify Analyze Health button visibility and functionality'
    )
    ...
    try {
          ...
          if (selector.includes(':contains')) {
            ...
            for (const button of buttons) {
              ...
              if (text.toLowerCase().includes('analyze')) {
                ...
                buttonFound = true
                console.log(
                  `🔘 Found Analyze Health button with text: "${text}"`
                )
                break
              }
            }
            ...
            if (buttonElement) {
              buttonFound = true
              console.log(
                `🔘 Found Analyze Health button with selector: ${selector}`
              )
              break
            }
          }
      ...
      if (buttonFound && buttonElement) {
        ...
        if (isVisible) {
          ...
          this.testResults.failed.push(
            '❌ Analyze Health button is not visible'
          )
        ...
        await this.takeScreenshot(
          '07-analyze-button-found',
          'Analyze Health button located'
        )
        ...
        if (isVisible && isEnabled) {
          ...
          if (
            newUrl.includes('/dashboard/health') ||
            newUrl.includes(this.expectedBusinessId)
          ) {
            this.testResults.passed.push(
              '✅ Button click navigates to health dashboard'
            )
            console.log(
              `✅ Button click navigates to health dashboard: ${newUrl}`
            )
          } else {
            this.testResults.warnings.push(
              `⚠️ Button click might not navigate correctly. Current URL: ${newUrl}`
            )
            console.log(
              `⚠️ Button click might not navigate correctly. Current URL: ${newUrl}`
            )
          ...
          await this.takeScreenshot(
            '08-after-button-click',
            'Page after clicking Analyze Health button'
          )
        }
        ...
        this.testResults.warnings.push(
          `⚠️ Available buttons: ${JSON.stringify(allButtons)}`
        )

        await this.takeScreenshot(
          '07-no-analyze-button',
          'Page when Analyze Health button not found'
        )
        return false
      }
    } catch (error) {
      this.testResults.failed.push(
        `❌ Button verification failed: ${error.message}`
      )
  ...
  async test6_VerifyDashboardNavigation() {
    ...
    try {
      ...
      if (currentUrl.includes('/dashboard/health')) {
        this.testResults.passed.push(
          '✅ Successfully navigated to health dashboard'
        )
        console.log(
          `✅ Successfully navigated to health dashboard: ${currentUrl}`
        )
        ...
        if (dashboardElements.length > 0) {
          this.testResults.passed.push(
            `✅ Dashboard content loaded with ${dashboardElements.length} relevant elements`
          )
          console.log(
            `✅ Dashboard content loaded with ${dashboardElements.length} relevant elements`
          )
        ...
        this.testResults.failed.push(
          `❌ Not on dashboard page. Current URL: ${currentUrl}`
        )
        ...
        await this.takeScreenshot(
          '09-direct-dashboard-nav',
          'Direct dashboard navigation attempt'
        )

        const newUrl = this.page.url()
        if (newUrl.includes('/dashboard/health')) {
          this.testResults.passed.push(
            '✅ Direct navigation to dashboard successful'
          )
          console.log('✅ Direct navigation to dashboard successful')
          return true
        } else {
          this.testResults.failed.push(
            '❌ Direct navigation to dashboard failed'
          )
      ...
      this.testResults.failed.push(
        `❌ Dashboard navigation verification failed: ${error.message}`
      )
      console.error(
        `❌ Dashboard navigation verification failed: ${error.message}`
      )
  ...
  async generateReport() {
    ...
    if (this.testResults.warnings.length > 0) {
      report.recommendations.push(
        '⚠️ Some warnings detected - review for potential improvements'
      )
    }
