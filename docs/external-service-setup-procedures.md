...
const SUBSCRIPTION_PLANS = [
  ...
  },
  {
    ...
    price: 29.99,
    interval: "month", 
    features: ["100 AI Analyses/month", "Professional Reports", "Portfolio Management", "Priority Support"],
  ...
  },
  {
    ...
    price: 99.99,
    interval: "month",
    features: ["Unlimited AI Analyses", "White-label Reports", "Bulk Analysis", "Dedicated Support"],
...
export const subscriptionWelcomeTemplate = (userData: User, planName: string) => ({
...
export const checkExternalServices = async () => {
  ...
  const results = await Promise.allSettled(
    services.map(async service => ({
      name: service.name,
      status: await service.check().then(() => 'healthy').catch(() => 'unhealthy')
    }))
  );
