...
async function testMarketplace() {
  ...
  try {
    ...
    if (businessCount > 0) {
      ...
      console.log(
        `   📋 Sample: "${sampleBusiness?.title}" by ${sampleBusiness?.owner.name}`
      )
    }
    ...
    console.log(
      `   ${testOwner ? '✅' : '❌'} Test business owner: ${testOwner ? 'Ready' : 'Missing'}`
    )
    console.log(
      `   ${testBuyer ? '✅' : '❌'} Test buyer: ${testBuyer ? 'Ready' : 'Missing'}`
    )
    ...
    try {
      const response = await fetch(
        'http://localhost:3000/api/businesses?page=1&limit=3'
      )
      if (response.ok) {
        const data = await response.json()
        console.log(
          `   ✅ API responded with ${data.businesses?.length || 0} businesses`
        )
      } else {
      ...
      console.log(
        '   ❌ API connection failed - make sure dev server is running'
      )
    }
    ...
    console.log(
      '      • Business Owner: testowner@goodbuyhq.com / TestOwner123!'
    )
