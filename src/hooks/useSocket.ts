...
export function useSocket() {
  ...
  const eventListenersRef = useRef<Map<keyof SocketEvents, Set<Function>>>(new Map())
  ...
  const removeEventListener = useCallback(<K extends keyof SocketEvents>(
    event: K,
    listener: SocketEvents[K]
  ) => {
    if (!socketRef.current) return
...
export function useMessageSocket(threadId: string | null) {
  const { addEventListener, removeEventListener, joinThread, leaveThread, startTyping, stopTyping } = useSocket()
  ...
  const onMessageUpdated = useCallback((callback: (data: any) => void) => {
...
export function useNotificationSocket() {
  ...
  const onMeetingInvitation = useCallback((callback: (data: any) => void) => {
    return addEventListener('meeting_invitation', callback)
  }, [addEventListener])

  const onDocumentShared = useCallback((callback: (data: any) => void) => {
