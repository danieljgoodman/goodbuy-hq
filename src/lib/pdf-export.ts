...
export class PDFExporter {
  ...
  private addHeader(result: ValuationResult): void {
    ...
    const reportDate = new Date(result.evaluationDate).toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'long',
      day: 'numeric'
    })
  ...
  private addExecutiveSummary(result: ValuationResult): void {
    ...
    this.doc.setFont('helvetica', 'bold')
    this.doc.text('Estimated Business Value:', this.margin + 5, this.yPosition + 5)
    
    this.doc.setFontSize(20)
    this.doc.setTextColor(14, 165, 233)
    this.doc.text(formatCurrency(result.overallValuation), this.margin + 5, this.yPosition + 15)
    
    this.doc.setTextColor(0, 0, 0)
    this.doc.setFontSize(12)
    this.doc.setFont('helvetica', 'normal')
    this.doc.text(`Confidence Score: ${result.confidenceScore.toFixed(0)}%`, this.margin + 100, this.yPosition + 15)
  ...
  private addValuationMethods(result: ValuationResult): void {
    ...
    result.methods.forEach((method, index) => {
      ...
      this.doc.setFont('helvetica', 'normal')
      this.doc.text(formatCurrency(method.value), this.margin + 100, this.yPosition)
      this.doc.text(`${method.confidence.toFixed(0)}% confidence`, this.margin + 150, this.yPosition)
  ...
  private addKeyMetrics(result: ValuationResult): void {
    ...
    const metrics = [
      { label: 'Revenue Multiple', value: `${result.keyMetrics.revenueMultiple.toFixed(1)}x` },
      { label: 'Profit Margin', value: `${result.keyMetrics.profitMargin.toFixed(1)}%` },
      { label: 'Return on Assets', value: `${result.keyMetrics.returnOnAssets.toFixed(1)}%` },
      { label: 'Debt-to-Equity', value: result.keyMetrics.debtToEquity.toFixed(2) },
      { label: 'Growth Rate', value: `${result.keyMetrics.growthRate.toFixed(1)}%` }
    ]
  ...
  private addRecommendations(result: ValuationResult): void {
    if (result.recommendations.length === 0 && result.riskFactors.length === 0) {
      return
    }
    ...
    if (result.recommendations.length > 0) {
      ...
      result.recommendations.forEach((recommendation, index) => {
        ...
        const lines = this.doc.splitTextToSize(`${index + 1}. ${recommendation}`, 160)
